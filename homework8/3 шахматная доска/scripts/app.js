let app = {
    ...
    //из-за большого числа повторов этих значений, чтобы уменьшить вероятноть ошибиться в наборе
    colors: {
        black: "black",
        white: "white",
    },
    ...
    insertPawns() {
        ...
        for (let i = 0; i < whitePawnsRow.length; i++) {
            ...
            whitePawnsRow[i].innerHTML = this.getFigure("pawn", this.colors.white + "Figure");
        }
        ...
        for (let i = 0; i < blackPawnsRow.length; i++) {
            ...
            blackPawnsRow[i].innerHTML = this.getFigure("pawn", this.colors.black + "Figure");
        }
    },
    ...
    generateBoard() {
        ...
        let rowStartWithColor = this.colors.white;
        ...
        for (let i = 0; i < this.config.rows.length; i++) {
            ...
            if (rowStartWithColor == this.colors.white) {
                ...
                rowStartWithColor = this.colors.black;

            } else {
                // 3.7 делаем то же что и в п. 1.5
                row = this.generateRow(rowStartWithColor, this.config.rows[i]);
                // 3.8 меняем rowStartWithColor на "white"
                rowStartWithColor = this.colors.white;
    ...
    generateRow(startWithColor, rowNum) {
        ...
        for (let i = 0; i < this.config.cols.length; i++) {
            ...
            if (currentColorClass == this.colors.white) {
                ...
                field = this.generateField(this.colors.white, rowNum, this.config.cols[i]);
                // 2.6 присваиваем в currentColorClass строку "black"

            } else {
                // 2.7 делаем то же что и в п. 2.5, только первым параметром передаем "black"
                field = this.generateField(this.colors.black, rowNum, this.config.cols[i]);

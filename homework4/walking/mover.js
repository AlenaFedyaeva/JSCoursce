let mover = {
    ...
    checkPosX(x) {
        if (x < 0 || x >= config.colsCount) {
            return false
        }
        return true
    },
    checkPosY(y) {
        if (y < 0 || y >= config.rowsCount) {
            return false
        }
        return true
    },
    /**
     * Проверяем точку в которую будет двигаться пользователь
     * @param {{x: int, y: int}} Point точка пользователя.
     */
    checkPos(point) {
        if (checkPosY(point.y) && checkPosX(point.x)) {
            return true
        }
        return false
    },
